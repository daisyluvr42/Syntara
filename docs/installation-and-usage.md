...
更新会拉取新版代码、更新依赖，并重新写入对应客户端的 MCP / Skill 入口；不会删除本地资料库、PDF、语料或风格档案。
...
Update pulls the latest code, updates dependencies, and rewrites the target MCP / skill entries. It does not delete local libraries, PDFs, corpora, or style profiles.
