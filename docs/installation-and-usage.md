...
python3 mcp/syntara.py install workbuddy \
  ...
  --style-name "Blog Article Style" \
  --style-project default \
  --style-type blog-article
```

也可以用交互式方式：

```bash
python3 mcp/syntara.py install workbuddy --setup-style
```
...
更新会拉取新版代码、更新依赖，并重新写入对应客户端的 MCP / Skill 入口；不会删除本地资料库、PDF、语料或风格档案。旧版的 `syntara-knowledge-writing`、`syntara-academic-writing`、`syntara-literature-review` 安装目录会在更新时清理，并替换为新的 `syntara-writing` 入口。
...
skills/syntara-writing
```

默认安装命令已经会把这两个 Skill 安装到 WorkBuddy，并写入用户导入元数据：
...
- `syntara-writing`：统一写作入口，适用于 ima、Syntara、腾讯文档或其他资料库来源，也覆盖文章、章节、文献综述、related work、研究差距、报告、脚本和提纲。
...
关键是为不同文体设置不同 `style_type`，并用通用体裁名而不是个人题材名。常用值包括 `blog-article`、`article`、`op-ed`、`literature-review`、`academic-paper`、`technical-report`、`business-report`、`white-paper`、`proposal`、`memo-email`、`documentation`、`instructional-guide`、`manual`、`presentation`、`talk-script`、`social-post` 或 `general`。Syntara 会把它们保存为不同风格档案，Skill 写作时按任务类型调用对应档案。

## 9. 在 WorkBuddy 中写作

推荐流程：

1. 选择 `syntara-writing`。
...
```text
使用 syntara-writing。项目是 implant-flap-review。
...
```text
使用 syntara-writing。项目是 professional-book。
...
To update Syntara and refresh a client integration later:
...
Update pulls the latest code, updates dependencies, and rewrites the target MCP / skill entries. It does not delete local libraries, PDFs, corpora, or style profiles. Older installed `syntara-knowledge-writing`, `syntara-academic-writing`, and `syntara-literature-review` directories are cleaned up during update and replaced by the new `syntara-writing` entry.

## 6. Install WorkBuddy Skills

The repository includes two user-visible skills:
...
- `syntara-writing`: unified source-based writing from ima, Syntara, Tencent Docs, or other knowledge-base materials. It covers articles, chapters, literature reviews, related work, research gaps, reports, scripts, and outlines.
...
Set a different `style_type` for each writing form, using generic genre names rather than personal topics. Common values include `blog-article`, `article`, `op-ed`, `literature-review`, `academic-paper`, `technical-report`, `business-report`, `white-paper`, `proposal`, `memo-email`, `documentation`, `instructional-guide`, `manual`, `presentation`, `talk-script`, `social-post`, or `general`. Syntara stores them as separate style profiles, and skills choose the matching profile for the current writing task.

## 9. Writing With WorkBuddy

Recommended workflow:

1. Choose `syntara-writing`.
...
```text
Use syntara-writing. The project is implant-flap-review.
...
```text
Use syntara-writing. The project is professional-book.
